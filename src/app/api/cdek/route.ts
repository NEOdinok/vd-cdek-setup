...
export async function GET(request: Request) {
  try {
    ...
    switch (action) {
      case "offices":
        [responseData, responseHeaders] = await getOffices(
          authToken,
          requestData
        );
        break;
      case "calculate":
        [responseData, responseHeaders] = await calculate(
          authToken,
          requestData
        );
        break;
...
async function getOffices(authToken: string, requestData: any) {
  const params = {
    ...
    is_handout:
      requestData.is_handout !== undefined ? requestData.is_handout : true,
    ...requestData,
  };
...
async function httpRequest(
  ...
  const url = useJson
    ? `${baseUrl}/${method}`
    : `${baseUrl}/${method}?${new URLSearchParams(data)}`;
